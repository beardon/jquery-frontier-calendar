...
(function($) {
	...
	function CalendarDayCell(jqyObj) {
		...
		*/
		...
		/**
		 * Add the "more" link div and renders it (if there is not already one);
		 */
		...
		this.getNextAgendaY = function(agendaDivHeight,moreDivHeight){
			var nextY = this.getY();
			return this.getNextAgendaYstartY(nextY,agendaDivHeight,moreDivHeight);
		};
		*/
		// sort a jquery object by Y coordinate
		this.sortDivByY = function(a,b){
			var y1 = a.position().top;
			var y2 = b.position().top;
			return ((y1 < y2) ? -1 : ((y1 > y2) ? 1 : 0));
		};

		/**
		 * Returns the next Y coordinate starting at start Y
		 *
		...
		/**
		 * Returns the next Y coordinate after (start Y coordindate + agendaDivHeight + 1) where another agenda <div> element could be rendered.
		 *
		 * @param starty - integer - The start Y coordinate. We start looking after this coordinate.
		 * @param agendaDivHeight - integer - Height for the new agenda <div> element.
		 * @param moreDivHeight - integer - Height for the "more" link <div> element. We always need to leave room for the "more" link.
		 * @return - integer - A Y coorindate where a next agenda <div> element could be rendered, or -1 if there is no space.
		 */
		/*
		this.getNextAgendaYafterY = function(startY,agendaDivHeight,moreDivHeight){
			var nextY = startY + agendaDivHeight + 1;
			var divArray = this.agendaDivHash.values();
			if(divArray != null && divArray.length > 0){
				// sort agenda <div> elements by their Y coordinates
				divArray.sort(this.sortDivByY);
		...
		*/
		...
		this.renderAgendaDivElement = function(agendaId,displayMessage,startDayObject,endDayObject,leftEnd,rightEnd){
			...
			if(nextY > 0){
				...
				//this.addMoreDivToDays(startDayObject,endDayObject);
		 ...
		 * @param moreDiv - jquery object - the more link <div> element, or null.
		 */
		this.addMoreDivToDays = function(startDayObj,endDayObj,moreDiv){
			...
			startDayObj.addMoreDiv(moreDiv);
		...
		this.addAgendaDivToDays = function(startDayObj,endDayObj,agendaDiv,agendaId){
			...
			var startDt = startDayObj.getDate();
			//alert("Added agenda div " + agendaId + " to day " + startDt);
			var endDt = endDayObj.getDate();
			var nextDt = DateUtil.getNextDay(startDt);
			while(DateUtil.daysDifferenceDirection(nextDt,endDt) >= 0){
				var nextDatObj = this.getCalendarDayObjByDate(nextDt);
				//alert("Adding agenda div " + agendaId + " to day " + nextDt);
