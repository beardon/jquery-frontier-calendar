...
(function($) {
	 ...
	 */
	Calendar.loadICalSource = function(cal,iCalUrl){
				...
				*/
				if(icalParser.ical.events.length > 0){
					...
					for(var i=0; i<icalParser.ical.events.length; i++){
						...
						startYear = event.dtstart.value.substring(0,4);
						startMonth = event.dtstart.value.substring(4,6).replace(/^[0]+/g,"");
						startDay = event.dtstart.value.substring(6,8).replace(/^[0]+/g,"");
						endYear = event.dtend.value.substring(0,4);
						endMonth = event.dtend.value.substring(4,6).replace(/^[0]+/g,"");
						endDay = event.dtend.value.substring(6,8).replace(/^[0]+/g,"");
						startDt = new Date(parseInt(startYear),parseInt(startMonth)-1,parseInt(startDay),0,0,0,0);
						endDt = new Date(parseInt(endYear),parseInt(endMonth)-1,parseInt(endDay),0,0,0,0);
						if(DateUtil.secondsDifferenceDirection(startDt,endDt) >= 0){
							...
							hashData.put('SUMMARY',summary);
							hashData.put('DTSTART',summary);
							hashData.put('DTEND',summary);
							hashData.put('UID',event.uid.value);
	...
	var jFrontierCalPlugin = function(calElm,dayClickCallback,agendaClickCallback){
		 ...
		 *
		 */
		this.addAgendaItem = function(calId,title,startDate,endDate,data,displayProp){
		...
		/**
		 * Loads the iCal data into the calendar.
		 *
		 * @param calId - (String) - The ID of the calendar </div> element.
		 * @param iCalUrl -(String) - URL for the iCal data.
		 */
		this.loadICalSource = function(calId,iCalUrl){
			if(calId != null && iCalUrl != null){
				calId = stripNumberSign(calId);
				var calObj = myCalendars.get(calId);
				Calendar.loadICalSource(calObj,iCalUrl);
			}
		};
