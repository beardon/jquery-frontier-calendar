 ...
 *
 * This plugin is yours! Do with as you want! I claim no responsibility if it explodes and ruins your day. :)
 *
 * Dependencies:
 *
...
(function($) {
	...
	function Calendar() {
		...
		*/
		...
		this.do_init = function(){
			...
			//alert(headerCellTotalHeight + " " + dayCellTotalHeight);
			...
			if(nextY > 0){
				...
				if(this.clickEvent_agendaCell != null){
					d.bind(
						'click',
						{
							// pass agenda ID so use has access to it.
							agendaId: agendaId
						},						
						this.clickEvent_agendaCell
					);
				}			
		...
		*/		
		...
		this.resize = function(){
			...
			//alert(headerCellTotalHeight + " " + dayCellTotalHeight + " " + weekHeaderCellTotalHeight);			
	...
	var jFrontierCalPlugin = function(calElm,dayClickCallback,agendaClickCallback){
		 ...
		 * }
		 *
		 * @return Javascript Object with the following data spec.
		 *         
		 * {
		 *     agendaId: [integer],
		 *     startDate: [Date],
		...
		this.getAgendaItem = function(calId,agendaId){
			if(calId != null && agendaId != null){
				calId = stripNumberSign(calId);
				var calObj = myCalendars.get(calId);
				return calObj.getAgendaItemById(parseInt(calId));
			}
		};
