 ...
 *
 * 2) WResize is the jQuery plugin for fixing the IE window resize bug.
 *    This plugin is included at the end of this file.
...
(function($) {
		...
		*/
		...
		this.renderSingleAgendaItem = function(agi){
			...
			if((DateUtil.daysDifference(firstRenderDate,lastRenderDate) + firstDtIndex) > 6){
				...
				var displayMessage = DateUtil.getAgendaDisplayTime(agi.getStartDate())+" " + agi.getTitle();
		...
		/**
		 * Called when users click an agenda item on the calendar. It simply calls the user specified
		...
		/**
		 * Called when users click and agenda item from the "more agenda items" modal dialog.
		 * This function close the dialog, the it calls this.clickAgendaFromCalendarHandler()
		 *
		 * @param eventObj - The event object from the click event. Should have the following values in its data.
		 *					 eventObj.data.callBack - The users custom click event callback function.
		 *					 eventObj.data.agendaId - The ID of the agenda item that was clicked.
		...
		this.addMoreDivToDays = function(startDayObj,endDayObj,moreDivHeight){
			...
			d.bind(
				...
				function(eventObj){
					...
					/*
					alert("you clicked the more link for day " + eventObj.data.calDayDate + ". There are " + 
					eventObj.data.agendaItems.length + " agenda items on this day.");
					*/
				}
			);
			// change mouse cusor to pointer when hovering over agenda divs.
		...
		this.resize = function(){
			...
			var cellWidthLast = cellWidth + ( calWidth - (cellWidth * Calendar.dayNames.length)) - (firstDayCell.jqyObj.outerWidth(true) - firstDayCell.jqyObj.width());
			...
			var cellHeight = cellWidth - headerCellHeight;
	...
	Calendar.showMoreAgendaModal = function(cal,date,agendaItems){
		...
		for(var i=0; i<agendaItems.length; i++){
			...
			agendaDiv.append("From " + DateUtil.getAgendaDisplayTime(agi.getStartDate()) + " on " + agi.getStartDate().toDateString() + 
				" To " + DateUtil.getAgendaDisplayTime(agi.getEndDate()) + " " + agi.getEndDate().toDateString()  + "<br>");
			// add click event
			if(cal.clickEvent_agendaCell != null){
				agendaDiv.bind(
					...
					cal.clickAgendaFromCalendarMoreModalDialog						
				);			
			}
			// change mouse cusor to pointer when hovering over agenda divs.
			agendaDiv.hover(
	...
	var jFrontierCalPlugin = function(calElm,dayClickCallback,agendaClickCallback){
		...
		/**
		 * Add agenda item to Calendar display and specifiy the background color and text color.
		 *
		 * @param calId     		- (String) - The ID of the calendar </div> element.
		 * @param title     		- (String) - The title of the agenda item.
		 * @param startDate 		- (Date)   - Date the agenda event starts.
		 ...
		 * @param data      		- (Object) - Any data you want to store in the agenda item. An object with a series of key value pairs.
		 *								 		 e.g. {var1: "some data", var2: "more data", var3: "etc..."}
		 * @param backgroundColor	- string   - Html color, e.g. "red", "rgb(255,0,0)", "#ff0000", etc... 
		 * @param foregroundColor	- string   - Html color, e.g. "red", "rgb(255,0,0)", "#ff0000", etc... 
		 */
		this.addAgendaItemWithColor = function(calId,title,startDate,endDate,data,backgroundColor,foregroundColor){
			if(calId != null && title != null && startDate != null && endDate != null && foregroundColor != null && backgroundColor != null){
				// make sure start date comes before end date
				if(DateUtil.secondsDifferenceDirection(startDate,endDate) < 0){
					alert("Sorry, you can't create an event that ends before it starts");
					return;
				...
				var agi = new CalendarAgendaItem(title,startDate,endDate,hashData);
				agi.setBackgroundColor(backgroundColor);
				agi.setForegroundColor(foregroundColor);
				var calObj = myCalendars.get(calId);
				calObj.addAgendaItem(agi);		
			}
		};	
		 ...
		 *
		 */
		this.getAgendaItem = function(calId,agendaId){
			if(calId != null && agendaId != null){
				calId = stripNumberSign(calId);
				var calObj = myCalendars.get(calId);
				var calAgendaItem = calObj.getAgendaItemById(parseInt(agendaId));
				if(calAgendaItem == null){
					alert("agenda item is null!");
				}
				var dataObj = null;
				var agendaItemHashData = calAgendaItem.getAgendaDataHash();
				...
				var agendaData = {
					agendaId: calAgendaItem.getAgendaId(),
					title: calAgendaItem.getTitle(),
					startDate: calAgendaItem.getStartDate(),
					endDate: calAgendaItem.getEndDate(),
					data: dataObj
				};
				//calAgendaItem.getAgendaDataHash()
				return agendaData;
			}
		};
		 ...
		 * @param agendaId - integer - The unique ID of the agenda item.
		 */
		this.deleteAgendaItem = function(calId,agendaId){
			if(calId != null && agendaId != null){
				calId = stripNumberSign(calId);
				var calObj = myCalendars.get(calId);
