...
(function($) {
	...
	function CalendarDayCell(jqyObj) {
		...
		/*
		each agenda <div> element has it's own position on the day cell, starting at 0 for the first and going up (2,3,4,etc...)
		in this hash we store the agenda <div> elements so we know if a particular position is taken or empty.
		if agendaRenderPositionsHash.get(0) returns null than that position is open
		if agendaRenderPositionsHash.get(0) returns a jquery object for the agenda <div> element than that position is full.
		keys are integers, values are jquery objects
		*/
		...
		/**
		 * Mark if an agenda rendering position is open or closed. Pass in a valid jquery object
		 * for an index to mark that index as taken, pass in null to mark it as open.
		 *
		 * @param index - integer - The position index, e.g. 0, 1, 2, etc.
		 * @param agendaDiv - Jquery object - a jquery object for the <div> element, or null.
		 */
		this.setAgendaPositionFlag = function(index,agendaDiv){
			this.agendaRenderPositionsHash.put(index,agendaDiv);
		};
		...
		/**
		 * Returns the next Y coordinate where another agenda <div> element could be rendered.
		 *
		 ...
		 */
		this.getNextAgendaY = function(agendaDivHeight,moreDivHeight){
			// get height of all agenda div elements currently on this day cell.
			var totalHeight = 0;
			var jqyObjDiv = null;
			var agendaDivsArray = this.agendaRenderPositionsHash.values();
			if(agendaDivsArray != null && agendaDivsArray.length > 0){
				for(var i = 0; i < agendaDivsArray.length; i++){
					jqyObjDiv = agendaDivsArray[i];
					if(jqyObjDiv != null){
						totalHeight += jqyObjDiv.height() + 1; // 1 for spacing
					}
				}
			}
			var agendaStartY = this.getY() + totalHeight;
			if( agendaStartY > ( (this.getY() + this.getHeight()) - (agendaDivHeight + 1) - (moreDivHeight + 1)) ){
		...
		/**
		 * Check to see of this day cell has enough room for another agenda item with the specified height.
		 *
		 * @param agendaDivHeight - integer = Height for the new agenda <div> element.
		 * @param moreDivHeight - integer = Height for the "more" link <div> element. We always need to leave room for the "more" link.	
		 * @return true if full, false if there is room.
		 */
		this.isAgendaViewFull = function(agendaDivHeight,moreDivHeight){			
			if(this.getNextAgendaY(agendaDivHeight,moreDivHeight) == -1){
				return true;
			}
			return false;
		};
	...
	function Calendar() {
		...
		*/
		...
		this.calBorderColor				= "#efefef";	// color of border arounc entire calendar
		...
		this.getAgendaItemsForDay = function(date){
			...
			for(var itemIndex = 0; itemIndex < this.agendaItems.size(); itemIndex++){
				...
				if(DateUtil.daysDifferenceDirection(startDt,date) >= 0 && DateUtil.secondsDifferenceDirection(endDt,date) <= 0){
					// push is not supported by IE 5/Win with the JScript 5.0 engine
		 ...
		 *				                   this is the start date of the agenda item. False not to round it.		 
		 */
		this.renderAgendaDivElement = function(displayMessage,startDayObject,endDayObject,leftEnd,rightEnd){
			...
			var startY = startDayObject.getY() + // location of day cell
				...
				(firstOpenIndex * 1); // plus a little spacing between each agenda item
			

			// create div for agenda item
			var d = $("<div/>");
			...
			// add sample click event to agenda div.
			/*
			d.bind('click', function(event) {
				alert('User clicked on "foo."');
				// prevent the even from bubbling down to the day cells under the agenda item
			...
			*/
			...
			// add the agenda <div> element to all the days it's being rendered on.
			this.flagAgendaPosition(firstOpenIndex,startDayObject,endDayObject,d);				
			
		};

		/**
		 * Sets the agenda renger position flag for start day, end day, and all days
		 * inbetween. Pass in a valid jquery object to mark position as take, pass
		 * in a null to mark it as open.
		 *
		 ...
		 */
		this.flagAgendaPosition = function(index,startDayObj,endDayObj,agendaDiv){
			if(index == null || startDayObj == null || endDayObj == null){
				return;
			}
			startDayObj.setAgendaPositionFlag(index,agendaDiv);
			...
			while(DateUtil.daysDifferenceDirection(nextDt,endDt) >= 0){
				...
				nextDatObj.setAgendaPositionFlag(index,agendaDiv);
		...
		/**
		 * Sets the heigh on all the days cells from start to end day.
		 *
		 * @param height - integer - The new heigh value.
		 * @param startDayObj - CalendarDayCell - The start day.
		 * @param endDayObj - CalendarDayCell - The end day.
		 */
		this.updateDayCellheights = function(height,startDayObj,endDayObj){
			if(height == null || startDayObj == null || endDayObj == null){
				return;
			}
			startDayObj.setHeight(height);
			var startDt = startDayObj.getDate();
			var endDt = endDayObj.getDate();
			var nextDt = DateUtil.getNextDay(startDt);
			while(DateUtil.daysDifferenceDirection(nextDt,endDt) >= 0){
				var nextDatObj = this.getCalendarDayObjByDate(nextDt);
				nextDatObj.setHeight(height);
				nextDt = DateUtil.getNextDay(nextDt);
			}			
		};		
