	...
	function Calendar() {
		...
		this.renderAgendaDivElement = function(agi,startDayObject,endDayObject,leftEnd,rightEnd){
			...
			var startY = startDayObject.getY() + // location of day cell
				(startDayObject.getAgendaRenderIndex() * this.agendaItemHeight) + // next location of agenda item
				(startDayObject.getAgendaRenderIndex() * 1); // plus a little spacing between each agenda item
			
			// increment agenda render indexes for start day, end day, and all days between!
			startDayObject.incrementAgendaRenderIndex();
			
			// get next day function
